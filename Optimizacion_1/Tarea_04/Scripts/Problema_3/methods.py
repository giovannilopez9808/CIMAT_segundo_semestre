...
class stop_functios_class:
    ...
    def functions(self, function: functions_lambda, xi: np.array, xj: np.array) -> bool:
        fxi = function.f(xi)
        fxj = function.f(xj)
...
class problem_class:
    ...
    def save(self):
        ...
        self.results.to_csv(join_path(self.parameters["path results"],
                                      filename))
...
class obtain_alpha():
    ...
    def bisection(self, function: functions_lambda, x: np.array, d: np.array) -> float:
        alpha = 0
        alpha_i = 1
        beta = np.inf
...
def obtain_armijo_conditon(function: functions_lambda, dot_grad: float, x: np.array, d: np.array, alpha: float, c1: float):
...
def obtain_wolfe_condition(function: functions_lambda, x: np.array, dot_grad: float, d: np.array, c2: float, alpha: float):
