...
parameters = datasets.obtain_dataset("rosembrock 100 random")
...
for file in files:
    ...
    color = colors[method]
    filename = join_path(parameters["path data"], file)
    data = read_data(filename)
    ax1.set_xlim(parameters["xlim"][0], parameters["xlim"][1])
    ax1.set_ylim(parameters["y1 lim"][0], parameters["y1 lim"][1])
    ax2.set_ylim(parameters["y2 lim"][0], parameters["y2 lim"][1])
    ax1.plot(data.index,
             data["fx"],
             label=method,
             color=color,
    ...
    ax2.plot(data.index,
             ...
             marker=".",
             ls="--")
    ax1.grid(ls="--", alpha=0.5, color="#000000")
